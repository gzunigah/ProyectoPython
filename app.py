...
ACONFIG=os.getenv('ACONFIG', './paises.ini')
...
@app.route("/ciudad/<string:id>/",methods=['DELETE'])
def borrar_ciudad(id):
  try:
    cur.execute("DELETE id, nombre, pais, capital FROM ciudad WHERE id=%s;", (id,))
    (id,) = cur.fetchone()
    cnx.commit()
    return jsonify({"id": id}), status.HTTP_201_CREATED
  except psycopg2.errors.UniqueViolation as e:
...
@app.route("/pais/<string:id>",methods=['DELETE'])
def borrar_pais(id):
  try:
    cur.execute("DELETE FROM pais WHERE id=%s;", (id,))
    (id,) = cur.fetchone()
    cnx.commit()
    return jsonify({"id": id}), status.HTTP_200_OK
  except psycopg2.errors.UniqueViolation as e:
    print("Error de llave duplicada:\n{}\n{}".format(type(e),str(e)))
    cnx.rollback()
    return jsonify({}), status.HTTP_409_CONFLICT
...
def mostrar_liderpaises():
  cur.execute("SELECT pais.nombre, lider.nombre, titulo, fecha_inicio, fecha_final, principal is true FROM pais, lider, lider_pais WHERE pais.id=lider_pais.pais AND lider.id=lider_pais.lider;")
  listaliderpaises=[]
  for (pais, lider, titulo, fecha_inicio, fecha_final, principal) in cur.fetchall():
    listaliderpaises.append({"pais": pais, "lider": lider, "titulo": titulo, "fecha_inicio": fecha_inicio, "fecha_final": fecha_final, "principal": principal})
  return jsonify(listaliderpaises), status.HTTP_200_OK
